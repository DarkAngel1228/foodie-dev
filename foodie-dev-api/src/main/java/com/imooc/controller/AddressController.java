...
public class AddressController {
    ...
    @ApiOperation(value = "用戶修改地址", notes = "用戶修改地址", httpMethod = "POST")
    @PostMapping("/update")
    public IMOOCJSONResult update(@RequestBody AddressBO addressBO) {
        if (StringUtils.isBlank(addressBO.getAddressId())) {
            return IMOOCJSONResult.errorMsg("修改地址錯誤：addressId不能爲空");
        }
        IMOOCJSONResult checkRes = checkAddress(addressBO);
        if (checkRes.getStatus() != 200) {
            return checkRes;
    ...
    @ApiOperation(value = "用戶刪除地址", notes = "用戶刪除地址", httpMethod = "POST")
    @PostMapping("/delete")
    ...
    @ApiOperation(value = "用戶設置默认地址", notes = "用户设置默认地址", httpMethod = "POST")
